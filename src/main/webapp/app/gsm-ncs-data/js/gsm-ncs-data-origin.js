...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择ncs文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
...
function doUpload() {
	...
	fileSize = 1;
	var fileCode = $("#formImportNcs #fileCode").val();
	var attachParams = {};
...
function displayNcsDescData(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		if(factory=="ERI"){
			...
			html+="<td>"+getValidValue(one['FREQ_SECTION'],'')+"</td>";			
