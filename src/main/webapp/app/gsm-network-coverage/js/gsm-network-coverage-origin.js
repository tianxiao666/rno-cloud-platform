$(document).ready(
    function () {
        ...
        $("#addInterMartix").click(function () {
            location.href = 'initDirectionAngleTaskAddForAjaxAction';
        });
    });
 ...
 * 按条件查询方向角计算任务
 */
function getDirectionAngleTask() {
    //重置分页条件
    initFormPage('interferMartixForm');
    //提交表单
    sumbitDirectionAngleTask();
}

/**
 * 提交表单
 */
function sumbitDirectionAngleTask() {
    //showOperTips("loadingDataDiv", "loadContentId", "正在加载");

    $("#interferMartixForm").ajaxSubmit({
        url: 'query2GDirectionAngleTaskForAjaxAction',
        dataType: 'text',
...
function showInterMartix(raw) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (getValidValue(one['WORK_STATUS'], '') == "计算完成") {
            tr += "<td>" + getValidValue(one['WORK_STATUS'], '')
                + ": <input type='button' value='下载结果文件' onclick='downloadDirAngleFile(\"" + one['JOB_ID'] + "\")'/>" + "</td>";
        } else {
...
function downloadDirAngleFile(jobId) {
    var form = $("#downloadDirAngleFileForm");
