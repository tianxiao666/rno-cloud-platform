$(function () {
    ...
    $("#exportCellBtn").on("click", function () {
        const param = $("#cell_targetParam").find("option:selected").val();
        if (param === "ALL") {
            param = "";
            for (let i = 0; i < cellParam.length; i++) {
                param += cellParam[i];
    ...
    $("#exportChannelBtn").on("click", function () {
        const param = $("#channel_targetParam").find("option:selected").val();
        if (param === "ALL") {
            param = "";
            for (let i = 0; i < channelParam.length; i++) {
    ...
    $("#exportNcellBtn").on("click", function () {
        const param = $("#ncell_targetParam").find("option:selected").val();
        if (param === "ALL") {
            param = "";
            for (let i = 0; i < ncellParam.length; i++) {
                param += ncellParam[i];
...
function uploadParamValue(cityId, type) {
    ...
    $.ajax({
        ...
        success: function (data) {
            let listInfo = {};
            listInfo = eval("(" + data + ")");
            let bscId, bscName, dateStr;
            for (let key in listInfo) {
                //bsc参数
                if ("bscInfo" === key) {
                    for (let bscIndex = 0; bscIndex < listInfo[key].length; bscIndex++) {
                ...
                //date参数
                if ("dateInfo" === key) {
                    for (let dateIndex = 0; dateIndex < listInfo[key].length; dateIndex++) {
...
function PutRightOneClk(defaul, select) {
    ...
    while (document.getElementById(defaul).options.selectedIndex > -1) {
        const id = document.getElementById(defaul).options.selectedIndex
        const varitem = new Option(document.getElementById(defaul).options[id].text, document.getElementById(defaul).options[id].value);
...
function PutRightAllClk(defaul, select) {
    ...
    for (let i = 0; i < document.getElementById(defaul).options.length; i++) {
        let varitem = new Option(document.getElementById(defaul).options[i].text, document.getElementById(defaul).options[i].value);
        document.getElementById(select).options.add(varitem);
    }
...
function PutLeftOneClk(defaul, select) {
    ...
    while (document.getElementById(select).options.selectedIndex > -1) {
        const id = document.getElementById(select).options.selectedIndex
        const varitem = new Option(document.getElementById(select).options[id].text, document.getElementById(select).options[id].value);
...
function PutLeftAllClk(defaul, select) {
    ...
    for (let i = 0; i < document.getElementById(select).options.length; i++) {
        const varitem = new Option(document.getElementById(select).options[i].text, document.getElementById(select).options[i].value);
        document.getElementById(defaul).options.add(varitem);
    }
...
function searchRecord(type) {
    let param = $("#" + type + "_targetParam").find("option:selected").val();
    if (param === "ALL") {
        param = "";
        if(type === "cell") {
            for (let i = 0; i < cellParam.length; i++) {
    ...
    const bsc = $("#" + type + "_targetBsc").val();
    const date = $("#" + type + "_targetDate").val();
    const cell = $("#" + type + "_targetCell").val().trim();
    const cityid = $("#" + "city-menu-" + type).find("option:selected").val();
    if(type === "cell") {
        typeFlag = cellFlag;
    }else if(type === "channel"){
    ...
    if (type === "cell" || type === "channel") {
        $.ajax({
            ...
            success: function (raw) {
                ...
                if (data.length === 0) {
                    ...
                    for (let p of param.split(",")) {
                        th += "<th>" + p + "</th>";
                        cloumn += "{'data': '" + p + "'},";
                    }
                    $("#" + type + "Table").append("<tr>" + th + "</tr>");
                    cloumn += "]";
                    const cloumnData = eval("(" + cloumn + ")");
                    $("#" + type + "ListTab").DataTable().clear();
        ...
        const ncell = $("#ncell_targetNcell").val().trim();
        $.ajax({
            url: "../../api/gsm-param-query/query-cell-param",
            data: {
                "cellParam": param,
            ...
            success: function (raw) {
                ...
                if (data.length === 0) {
                    ...
                    ncellFlag = true;
                    let th = "<th>日期</th>" + "<th>MSC</th>" + "<th>BSC</th>" + "<th>CELL</th>" +
                        "<th>N_BSC</th>" + "<th>N_CELL</th>" ;
                    let cloumn = "[{'data': 'MEA_DATE'},{'data': 'MSC'}," +
