...
function showCell(data) {
	...
	if (celllist) {
		...
		for (var i=0;i<celllist.length;i++) {
			...
			tr += "<td>" + one[0] + "</td>";
			tr += "<td>" + one[1] + "</td>";
			tr += "<td>" + getValidValue(one[2]) + "</td>";
			tr += "<td>" + getValidValue(one[3]) + "</td>";
			tr += "<td>" + getValidValue(one[4]) + "</td>";
			tr += "<td>" + getValidValue(one[5]) + "</td>";
			tr += "<td>" + getValidValue(one[6]) + "</td>";
...
function getResource() {
	...
	$.ajax({
				url : 'data/cell-import-list.json',
				dataType : 'json',
				success : function(data) {
					// alert("查询到数据。");
					console.log(data);
