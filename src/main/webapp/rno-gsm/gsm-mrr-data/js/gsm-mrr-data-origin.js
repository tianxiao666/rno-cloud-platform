...
function queryImportDataRec(){
	$("#searchImportForm").ajaxSubmit({
		...
		success:function(raw){
			...
			setPageView(data['page'],"mrrImportRecPageDiv");
		}
	});
}
...
function queryMrrDescData(){
	$("#searchMrrForm").ajaxSubmit({
		...
		success:function(raw){
			...
			setPageView(data['page'],"mrrListPageDiv");
		}
	});
}
...
function displayMrrDescData(data){
	...
	/*	console.log("hello");*/
		...
		if(ENfactory==="ERI"){
			...
			$("#tspeed").hide();
			html += "<tr>";
			html+="<td>"+getValidValue(one['CITY_NAME'],'')+"</td>";
			html+="<td>"+getValidValue(one['MEA_TIME'],'')+"</td>";
			/*html+="<td>"+CHfactory+"</td>";*/
...
function queryMrrDetailData() {
	...
	$("#searchMrrDetailForm").ajaxSubmit({
		url:'queryEriMrrDetailV2AjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
...
function returnToMrrList(){
	...
	$("#mrrDescListDiv").css("display","block");
}